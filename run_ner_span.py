...
from processors.utils_ner import bert_extract_predict_item
...
def predict(args, model, tokenizer, prefix="",batch_size=64):
    pred_output_dir = args.output_dir
    if not os.path.exists(pred_output_dir) and args.local_rank in [-1, 0]:
        os.makedirs(pred_output_dir)
    test_dataset,texts = load_and_cache_examples(args, args.task_name, tokenizer, data_type='test')
    ...
    results = []
    import time #start = time.time()#results = []
    start = time.time()#results = []
    output_predict_file = os.path.join(pred_output_dir, prefix, "test_predict.json")
    pbar = ProgressBar(n_total=len(test_dataloader), desc="Predicting")
    for step, batch in enumerate(test_dataloader):
        model.eval()
        #print(len(texts[step*batch_size:min(step*batch_size+batch_size,len(texts))]))#model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        with torch.no_grad():
        ...
        #print(start_logits.shape)#, end_logits = outputs[:2]
        R = bert_extract_predict_item(start_logits, end_logits)
        #print(R)#= bert_extract_item(start_logit, end_logits)
        if R:
            label_entities =[]
            for x in R:
                label_entity = []
                if x and len(x)>0 :
                    for d in x:
                        if len(d)>0:
                            label_entity.append([args.id2label[d[0]], d[1], d[2]])# if x  else [] for x in R]
                label_entities.append(label_entity)
        # else:
        #     label_entities = []
        # print(label_entities)# = [[args.id2label[x[0]], x[1], x[2]] for x in R]
        # print(len(label_entities))# = [[args.id2label[x[0]], x[1], x[2]] for x in R]
        #sys.exit(1)#print(len(label_entities))# = [[args.id2label[x[0]], x[1], x[2]] for x in R]
        for idx in range(step*batch_size,min(step*batch_size+batch_size,len(texts))):
            json_d = {}
            json_d['id'] = idx
            json_d['text'] =texts[idx] 
            json_d['entities'] = label_entities[idx-step*batch_size]
            results.append(json_d)
        pbar(step)
    logger.info("\n")
    print("time cost="+str(time.time()-start))#json_to_text(output_submit_file, test_submit)
...
def main():
    ...
    # Training
    if args.do_train:
        train_dataset,texts = load_and_cache_examples(args, args.task_name, tokenizer, data_type='train')
